...
export default class RecordsModel {
    ...
    exportCSV(userId) {
        ...
        const headers = ['日期', '模式', '文章', '速度', '净速度', '准确率', 'KPM', '用时', '正确', '错误', '改正', '退格', '击键'];

        const rows = records.map(r => {
            const stats = r.stats || {};
            const isChinese = r.mode === 'practice-cn' || r.mode === 'input-cn';
            const totalCorrect = (stats.correct || 0) + (stats.fixed || 0);
            const processed = (stats.correct || 0) + (stats.errors || 0) + (stats.fixed || 0);
            const minutes = (stats.elapsed || 0) / 60;
            const accuracy = processed === 0 ? 100 : Math.round((totalCorrect / processed) * 100);
            const speed = minutes > 0
                ? (isChinese ? Math.round(totalCorrect / minutes) : Math.round((totalCorrect / 5) / minutes))
                : 0;
            ...
            return [
                ...
                r.mode === 'practice-cn' ? '中文练习' : r.mode === 'practice-en' ? '英文练习' : r.mode,
                r.articleTitle || '',
                speed,
                netSpeed,
                accuracy,
    ...
    getSummary(userId) {
        const records = this.getByUser(userId);
        if (records.length === 0) {
            return { total: 0, avgSpeed: 0, maxSpeed: 0, avgAccuracy: 0, totalCorrect: 0, totalErrors: 0 };
        }

        let totalSpeed = 0, maxSpeed = 0, totalAccuracy = 0, totalCorrect = 0, totalErrors = 0;
        ...
        records.forEach(r => {
            ...
            const processed = (stats.correct || 0) + (stats.errors || 0) + (stats.fixed || 0);
            const minutes = (stats.elapsed || 0) / 60;
            const accuracy = processed === 0 ? 100 : Math.round((totalCorrectVal / processed) * 100);
            const speed = minutes > 0
                ? (isChinese ? Math.round(totalCorrectVal / minutes) : Math.round((totalCorrectVal / 5) / minutes))
                : 0;
    ...
    formatRecord(record) {
        ...
        const isChinese = record.mode === 'practice-cn' || record.mode === 'input-cn';
        ...
        return {
            ...
            backspaceRate: (stats.keystrokes || 0) > 0 ? Math.round(((stats.backspaces || 0) / (stats.keystrokes || 0)) * 100) : 0,
            kspc: totalCorrect > 0 ? parseFloat(((stats.keystrokes || 0) / totalCorrect).toFixed(2)) : 0,
            netKeystrokes: (stats.keystrokes || 0) - (stats.backspaces || 0),
            fixRate: processed > 0 ? Math.round(((stats.fixed || 0) / processed) * 100) : 0,
            fixSuccessRate: ((stats.errors || 0) + (stats.fixed || 0)) > 0 ? Math.round(((stats.fixed || 0) / ((stats.errors || 0) + (stats.fixed || 0))) * 100) : 0,
            charsPerBackspace: (stats.backspaces || 0) > 0 ? parseFloat((processed / (stats.backspaces || 0)).toFixed(1)) : 0,
        };
    }

    getRecentComparison(records, currentSpeed) {
        ...
        return {
            ...
            avg: speeds.length > 0 ? Math.round(speeds.reduce((a, b) => a + b, 0) / speeds.length) : 0,
            rank: speeds.length > 0 ? speeds.filter(s => s > currentSpeed).length + 1 : 0,
            total: speeds.length
        };
    }
}