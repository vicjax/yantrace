...
export default class ArticleView {
    ...
    _renderList() {
        ...
        this.listEl.innerHTML = this.articles.map(article => {
        ...
        if (this.contentEl) {
            if (editable) {
                ...
                if (!this.contentEl.textContent.trim()) {
                    this.contentEl.innerHTML = '<span class="placeholder">请从左侧选择一篇文章</span>';
                }
            }
        }
    }
    ...
    _updateButtons() {
        ...
        if (this.viewBtn) this.viewBtn.style.display = isEditing ? 'none' : 'inline-block';
        if (this.editBtn) this.editBtn.style.display = isEditing ? 'none' : 'inline-block';
        if (this.deleteBtn) this.deleteBtn.style.display = isEditing ? 'none' : 'inline-block';
        ...
        if (this.newBtn) this.newBtn.style.display = isEditing ? 'none' : 'inline-block';
    }
